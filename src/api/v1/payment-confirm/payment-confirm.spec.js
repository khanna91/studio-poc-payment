...
describe('Test paymentConfirm', () => {
  ...
  test('should able to confirm the payment', () => {
    PaymentService.updatePaymentStatus = jest.fn(() => Promise.resolve({ id: 1, status: 'CONFIRMED' }));
    OrderService.updateStatus = jest.fn(() => Promise.resolve(true));
    const status = jest.spyOn(res, 'status');
    const json = jest.spyOn(res, 'json');

    return controller.paymentConfirm(req, res)
      .then(() => {
  ...
  test('should able to confirm the payment but not order', () => {
    PaymentService.updatePaymentStatus = jest.fn(() => Promise.resolve({ id: 1, status: 'CONFIRMED' }));
